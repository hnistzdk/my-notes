...
![1555902055367](http://zdk-blog-image.test.upcdn.net//images/202203011852176.png)
...
- 虽然索引大大提高了查询速度，同时却会降低更新表的速度。如对表进行INSERT、UPDATE和DELETE。因为更新表时，MySQL不仅要保存数据，还要保存一下索引文件每次更新添加了索引列的字段，都会调整因更新所带来的键值变化后的索引信息。
- 索引只是提高效率的一个因素，如果MySQL有大数据量的表，就需要华时间研究建立最优秀的索引，或优化查询
...
- 创建
  ...
  ![1551438009843](http://zdk-blog-image.test.upcdn.net//images/202203011853312.png)
...
4. 频繁更新的字段不适合创建索引，因为每次更新不只是更新了记录，还要更新索引
5. where条件里用不到的字段不创建索引
6. 单键/组合索引的选择问题？(高并发下倾向创建组合索引)
7. 查询中用来排序的字段。排序字段若通过索引去访问将大大提高排序速度
8. 查询中用于统计或者分组的字段
...
2. 经常增删改的表
   ...
   > 原因：虽然建立索引提高了查询速度，但同时却会降低更新表的速度，如对表进行INSTERT、UPDATE和DELETE。
   >
   > 因为更新表时，MySQL不仅要保存数据，还要保存一下索引文件

3. 数据重复且分布平均的表字段，因此应该只为最经常查询和最经常排序的数据列建立索引。

4. 如果某个数据列包含许多重复内容，为它建立索引就没有太大的实际效果
...
![1555944126588](http://zdk-blog-image.test.upcdn.net//images/202203011852044.png)

2). 插入H，n>4，中间元素G字母向上分裂到新的节点

![1555944549825](http://zdk-blog-image.test.upcdn.net//images/202203011852993.png)

3). 插入E，K，Q不需要分裂

![1555944596893](http://zdk-blog-image.test.upcdn.net//images/202203011852144.png)

4). 插入M，中间元素M字母向上分裂到父节点G

（M 在 K、N中间，BTree最多含有n-1个key，这里即4个key，5个指针）

![1555944652560](http://zdk-blog-image.test.upcdn.net//images/202203011852572.png)

5). 插入F，W，L，T不需要分裂

![1555944686928](http://zdk-blog-image.test.upcdn.net//images/202203011852751.png)

6). 插入Z，中间元素T向上分裂到父节点中

（插入Z后是，NQTWZ，把中间T提出来，方便更快的查询，所以不提出Z）

![1555944713486](http://zdk-blog-image.test.upcdn.net//images/202203011852066.png)

7). 插入D，中间元素D向上分裂到父节点中。然后插入P，R，X，Y不需要分裂

![1555944749984](http://zdk-blog-image.test.upcdn.net//images/202203011852050.png)

8). 最后插入S（R后面），NPQR节点n>5，中间节点Q向上分裂，但分裂后父节点DGMT的n>5，中间节点M向上分裂

![1555944848294](http://zdk-blog-image.test.upcdn.net//images/202203011852929.png)
...
![image-20210520190717898](http://zdk-blog-image.test.upcdn.net//images/202203011852242.png)
...
![1555906287178](http://zdk-blog-image.test.upcdn.net//images/202203011852561.jpeg)
...
![1555906287178](http://zdk-blog-image.test.upcdn.net//images/202203011852063.png)
...
![image-20210520211641159](http://zdk-blog-image.test.upcdn.net//images/202203011852428.png)
...
![image-20210520211835491](http://zdk-blog-image.test.upcdn.net//images/202203011852279.png)

![image-20210520212305695](http://zdk-blog-image.test.upcdn.net//images/202203011852375.png)
